...
ds_high_les = 5.0               # dx, dy, dz that you want these high-res files at
...
ds_low_les  = 25.0               # dx, dy, dz of low-res files
# extent_low  = [3, 80, 3, 80, 20]   # extent in xmin, xmax, ymin, ymax, zmax, in D
extent_low = None


# ----------- Execution parameters
ffbin = '/Users/agupta/Projects/Tools/AG_openfast/build_v3p5p1_double/glue-codes/fast-farm/FAST.Farm'
# ffbin = '/Users/agupta/Projects/Tools/AG_openfast/build_v3p5p1/glue-codes/fast-farm/FAST.Farm'
...
ADfilename              = 'NREL-2p8-127_AeroDyn15.T.dat'
ADbladefilename         = 'NREL-2p8-127_AeroDyn15_blade.dat'
ADskfilename            = 'unused'
SubDfilename            = 'unused'
...
libdisconfilepath       = os.path.join('/Users/agupta/Projects/Tools/FlorisTools',templatePath,'libdiscon.dylib')
...
case.setTemplateFilename(templatePath, EDfilename, SEDfilename, HDfilename, SrvDfilename, ADfilename, ADbladefilename,
...
# TurbSim setup
turbsimexe = '/Users/agupta/Projects/Tools/AG_openfast/build_v3p4p1/modules/turbsim/turbsim'
# turbsimexe = '/Users/agupta/Projects/Tools/AG_openfast/build_v3p5p1_double/modules/turbsim/turbsim'
if LESpath is None:
    case.TS_low_setup()
...
for cond in range(case.nConditions):
    for casei in range(case.nCases):
        for seed in range(case.nSeeds):
            ...
            os.system('cp -r ' + os.path.join(templatePath,'Airfoils') + ' ' + os.path.join(case.path,case.condDirList[cond],case.caseDirList[casei]))
            os.system('cp -r ' + os.path.join(templatePath,'NREL-2p8-127_Cp_Ct_Cq.txt') + ' ' + os.path.join(case.path,case.condDirList[cond],case.caseDirList[casei]))
            # case.plot()
